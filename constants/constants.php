...
class constants {
    ...
    function getDateInThePastErrorText() {
        return "Дата находится в прошлом, либо не соотвествует условиям переноса. Дата, на которую переносится отпуск не должна быть равна дате начала переносимого отпуска и должна быть не менее 5 дней от текущей даты или даты начала переносимого отпуска. Введите корректную дату";
    }
