...
//
    ...
    function setSelectedDmsPollOptionForMultipleChoose($userId, $text, $pollQuestionInfo) {
        ...
        if ($result != null && (mysqli_num_rows($result) >= 1 )) {
            ...
            foreach ($responses['options'] as $key=>$value) {
                if ($value['id'] != (int)$replyInfo['selectedReplyId']) {
                    array_push($updatedResponsesList, $value);
                } else if ($value['id'] == (int)$replyInfo['selectedReplyId']) {
            ...
            $statement->bind_param("sii", $userId, $replyInfo['pollId'], $replyInfo['questionId']);
            $returnValue = $statement->execute();
        }
        return $returnValue;
    }
